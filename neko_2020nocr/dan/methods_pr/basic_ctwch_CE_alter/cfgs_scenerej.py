...
class scene_cfg:
    def __init__(self, datasetname="dictrej500.pt"):
        ...
        self.saving_cfgs = get_save_cfgs(prefix)
...
class scene_cfg_te(scene_cfg):
    def __init__(self, pathoveride, datasetname="dictrej500.pt"):
        dataset = DSCFG(cnt, T, datasetname, DSROOT)
        PDICT = dataset["dict_dir"]
        self.net_cfgs = get_net(PDICT, prefix, None, maxT=T)
        self.optimizer_cfgs = get_dos_optim()
        self.saving_cfgs = get_save_cfgs(prefix, pathoveride)
        self.dataset_cfgs = dataset
        self.loss_weight = loss[1]
        self.global_cfgs = get_test_cfg()
        self.net_cfgs = get_net(PDICT, prefix, "E9", maxT=T, root_override=pathoveride)
