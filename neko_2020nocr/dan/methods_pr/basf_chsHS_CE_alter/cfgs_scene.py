...
class scene_cfg_tejp(scene_cfg):
    def __init__(self, meta=None, root_override=None):
        self.global_cfgs = get_test_cfg()
        self.net_cfgs = get_net(pdict_evaljap(DSROOT), prefix, "E4", maxT=T, root_override=root_override)
        self.optimizer_cfgs = get_dos_optim()
        self.saving_cfgs = get_save_cfgs(prefix, root_override=root_override)
        self.dataset_cfgs = get_jap_test(T, None, DSROOT, meta)
