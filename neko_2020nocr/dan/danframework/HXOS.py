...
from neko_2020nocr.dan.common.common import display_cfgs, load_dataset, Zero_Grad, Train_or_Eval, generate_optimizer, \
    Updata_Parameters, flatten_label
...
class HXOS(neko_abstract_DAN):
    ...
    def runtest(self, miter=1000, debug=False, dpgpath=None, measure_rej=False):
        # print(self.test_acc_counter)
        # input()
        tools = [self.test_acc_counter,
                 flatten_label,
                 ]
        # print(measure_rej)
        # input()
        if measure_rej:
            tools = [None,
                     flatten_label,
                     self.test_rej_counter
                     ]
        # print("pause before test")
        # input()
        self.test(self.test_loader, self.model, tools, miter=miter, debug=debug, dbgpath=dpgpath)
